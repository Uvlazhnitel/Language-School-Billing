...
func (_q *ContactQuery) QueryStudentContacts() *StudentContactQuery {
...
func (_q *ContactQuery) WithStudentContacts(opts ...func(*StudentContactQuery)) *ContactQuery {
	query := (&StudentContactClient{config: _q.config}).Query()
...
func (_q *ContactQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Contact, error) {
	...
	if query := _q.withStudentContacts; query != nil {
		if err := _q.loadStudentContacts(ctx, query, nodes,
			func(n *Contact) { n.Edges.StudentContacts = []*StudentContact{} },
			func(n *Contact, e *StudentContact) { n.Edges.StudentContacts = append(n.Edges.StudentContacts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ContactQuery) loadStudentContacts(ctx context.Context, query *StudentContactQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *StudentContact)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(studentcontact.FieldContactID)
	}
	query.Where(predicate.StudentContact(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(contact.StudentContactsColumn), fks...))
	}))
